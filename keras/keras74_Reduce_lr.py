...
#3. 컴파일 훈련
from tensorflow.keras.callbacks import EarlyStopping, TensorBoard, ModelCheckpoint, ReduceLROnPlateau # 조기 종료, 텐서보드 임포트
...
ck = ModelCheckpoint("./model", monitor='val_loss', save_best_only=True, save_weights_only=True, mode='min', verbose=1)

# 러닝레이트를 감소시킬거야 3번에 걸쳐서 변화값의 개선이 없으면 러닝레이트를 50% 감축 시킬거야
...
model.fit(x_train, y_train, epochs=30, batch_size=32, verbose=1, validation_split=0.2, callbacks=[es,ck]) #원래 배치 사이즈의 디폴트는 32
