...
#3. 컴파일 훈련
# modelpath = './model/mnist-{epoch:02d}-{val_loss:4f}.hdf5' #d는 두자리 정수 f는 소수 네자리까지
# from tensorflow.keras.callbacks import EarlyStopping, ModelCheckpoint # 조기 종료
...
# hist = model.fit(x_train, y_train, epochs=30, batch_size=32, verbose=1, validation_split=0.2, callbacks=[es, cp]) #원래 배치 사이즈의 디폴트는 32
...
'''
save weight 처음 돌린 값
loss :  0.07685725390911102
acc :  0.9854999780654907
실제값 :  [7 2 1 0 4 1 4 9 5 9]
예측값 :  [7 2 1 0 4 1 4 9 6 9]
'''
'''
save weight 처음 돌린 값을 로드해서 훈련 시킨 값
loss :  0.07685725390911102
acc :  0.9854999780654907
실제값 :  [7 2 1 0 4 1 4 9 5 9]
예측값 :  [7 2 1 0 4 1 4 9 6 9]
'''
#결론 세이브 로드를 통해 일정한 값을 얻을 수도 저장시킬 수도 있다
