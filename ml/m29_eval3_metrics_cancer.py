...
model = XGBClassifier(n_estimators=700, learning_rate=0.4)
...
# 단일분류일때는 error : 이진 분류 오류율 (cancer는 단일분류)
model.fit(x_train, y_train, verbose=1, eval_metric=['error', 'error@5'], eval_set=[(x_train, y_train), (x_text, y_test)])
